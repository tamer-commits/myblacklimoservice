...
// Twilio's plain-SMS inbound webhook — SMS ONLY, do not confuse with the
// separate (shelved, not currently wired up) WhatsApp version of this same
// intake bot at app/api/whatsapp/webhook/route.js. Configure this URL in
// Twilio Console > Phone Numbers > Manage > Active Numbers > (the SMS-sending
// number, TWILIO_SMS_FROM) > Messaging Configuration > "A message comes in"
// as an HTTP POST to https://www.myblacklimoservice.com/api/sms/webhook.
//
// Tamer decided not to pursue WhatsApp Business API (it requires Meta
// Business Manager verification he doesn't want to do), so this SMS version
// is what actually goes live — same conversation, same question order, just
// over plain SMS on the Twilio number that's already live in production.
// The WhatsApp code/table is left in place, untouched, in case it's ever
// revisited.
//
...
// booking row with status AWAITING_MANUAL_QUOTE and source 'sms' for a human
// (Tamer/staff) to price and confirm manually. Nothing here quotes a fare or
// takes a payment.
//
// State machine: sms_intakes.state tracks which question we're waiting on an
// answer for. Each inbound message either (a) starts a brand-new session
// (send the greeting + first question, don't consume the message as an
// answer), (b) resets on a stale (24h+) or "restart" session, or (c) is
...
// — SMS and WhatsApp use the identical TwiML verb for a reply, so this file
// looks a lot like the WhatsApp webhook on purpose; the only real
// differences are the Twilio "From" number format (no "whatsapp:" prefix
// here) and the intake table/source used. The email OTP code itself still
// goes out over a different channel (Resend, via lib/otp.js's startEmailOtp).
...
// detection as the web quote/booking flow (app/api/quote/route.js) so all
// three intake paths (web, WhatsApp, SMS) never drift apart.
...
function questionFor(state, intake){
 switch(state){
  case 'ASK_NAME':
   return "Hi! Thanks for texting My Black Limo Service 🚘 Let's get your booking request started.\n\nWhat's your full name?";
  case 'ASK_ORIGIN':
...
async function finalizeBooking(phone, intake){
 ...
 notesParts.push('(submitted via SMS intake bot)');
 ...
 await upsertSmsIntake(phone, { state: 'DONE', booking_id: booking.id });
...
export async function POST(req){
 try{
  ...
  // Plain SMS inbound: Twilio sends "From" as a bare E.164 number here (no
  // "whatsapp:" prefix like the WhatsApp webhook gets) — strip it defensively
  // anyway in case Twilio ever sends a prefixed value on this route.
  ...
  let intake = await getSmsIntake(phone);
  let isNewSession = false;
  if(!intake){
   intake = await upsertSmsIntake(phone, { state: 'ASK_NAME' });
   isNewSession = true;
  } else {
   const ageMs = Date.now() - new Date(intake.updated_at).getTime();
   if(ageMs > SESSION_TIMEOUT_MS){
    intake = await resetSmsIntake(phone);
    isNewSession = true;
   }
  }
  ...
  if(intake.state === 'DONE'){
   return twiml(
    `Your booking request is already with our team — we'll be in touch shortly. Reply RESTART if you'd like to submit a new request.`
   );
  }
  ...
  switch(intake.state){
   case 'ASK_NAME': {
    ...
    intake = await upsertSmsIntake(phone, { name: clean(body, 100), state: 'ASK_ORIGIN' });
   ...
   case 'ASK_ORIGIN': {
    ...
    intake = await upsertSmsIntake(phone, { origin: clean(body, 250), state: 'ASK_DESTINATION' });
   ...
   case 'ASK_DESTINATION': {
    ...
    intake = await upsertSmsIntake(phone, { destination: clean(body, 250), state: 'ASK_DATETIME' });
   ...
   case 'ASK_DATETIME': {
    ...
    intake = await upsertSmsIntake(phone, { pickup_at_text: clean(body, 200), state: 'ASK_PASSENGERS_VEHICLE' });
    return twiml(questionFor('ASK_PASSENGERS_VEHICLE', intake));
   }

   case 'ASK_PASSENGERS_VEHICLE': {
    intake = await upsertSmsIntake(phone, {
   ...
   case 'ASK_SECOND_PHONE': {
    ...
    intake = await upsertSmsIntake(phone, { second_phone: clean(body, 40), state: 'ASK_EMAIL' });
   ...
   case 'ASK_EMAIL': {
    ...
    intake = await upsertSmsIntake(phone, { email, state: 'ASK_EMAIL_OTP' });
    const sent = await startEmailOtp(email, 'sms_email');
    if(!sent.ok && sent.reason !== 'cooldown'){
     ...
     intake = await upsertSmsIntake(phone, { state: fallbackState });
   ...
   case 'ASK_EMAIL_OTP': {
    if(lowerBody === 'resend'){
     const sent = await startEmailOtp(intake.email, 'sms_email');
     if(!sent.ok){
    ...
    const result = await verifyEmailOtp(intake.email, 'sms_email', body.trim());
    if(!result.ok){
     const msgs = {
    ...
    intake = await upsertSmsIntake(phone, { email_verified_at: new Date().toISOString(), state: nextState });
   ...
   case 'ASK_FLIGHT': {
    ...
    intake = await upsertSmsIntake(phone, { flight_number: flight, state: 'DONE' });
  ...
  console.error('SMS_WEBHOOK_ERROR', e);
...
export async function GET(){
 return NextResponse.json({ ok: true, note: 'This endpoint accepts POST requests from Twilio (SMS inbound webhook).' });
}
