...
// Nothing in this file runs for a booking still in a provisional status
// (AWAITING_MANUAL_QUOTE — an unpriced WhatsApp/SMS quote in progress, or
// AWAITING_PAYMENT — priced but not yet paid/manually confirmed) — see
// PROVISIONAL_STATUSES below.
//
// --- Checkpoint timeline -----------------------------------------------
...
//   T-2h    customer_escalate_unconfirmed — if STILL unconfirmed at this
//                                   same checkpoint, alert management to
//                                   call the customer directly, so there is
//                                   still a full 2 hours of runway left
//
...
// Bookings that haven't been finalised yet — still a WhatsApp/SMS quote in
// progress (no real price or pickup time) or still waiting on the customer
// to actually pay/be manually confirmed by Tamer — must never be swept into
// this workflow. Sending a driver a confirm-request, or a customer a
// "confirm your ride" slide-link, for a booking that might not even happen
// yet (payment abandoned, quote never finalised) would be confusing at best
// and would burn a driver's trust at worst. Nothing below runs, including
// auto-assigning a driver, until the booking leaves this set (normally by
// Tamer setting status to CONFIRMED once it's paid/finalised).
const PROVISIONAL_STATUSES = new Set(['AWAITING_MANUAL_QUOTE', 'AWAITING_PAYMENT']);

export function computeDueActions(booking, now = new Date()){
 const actions = [];
 if (booking.status === 'CANCELLED') return actions;
 if (PROVISIONAL_STATUSES.has(booking.status)) return actions;
 ...
 // Final deadline for the customer to confirm is T-2h, the same moment as
 // the last customer-facing reminder above — matches the requested
 // behaviour exactly ("if the customer still hasn't confirmed by this
 // point, trigger management intervention") rather than adding an extra,
 // undocumented grace hour before paging Tamer.
 if (!booking.customer_confirmed_at && !booking.customer_manual_dispatch_at && minutesToPickup <= 120) {
...
// The customer never confirmed at 48h or 24h — final deadline is the same
// T-2h checkpoint as the last reminder, giving management a full 2 hours to
// call the customer directly before the driver is dispatched/en route.
async function executeCustomerEscalate(booking){
 await flagManualDispatch(booking.id, 'customer_unconfirmed_2h_before_pickup');
