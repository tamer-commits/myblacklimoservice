...
export async function ensureSchema(){
 ...
 schemaReady = (async () => {
  ...
  // bookings already existed before user accounts did — add the column for
  // deployments that created the table on an earlier version of this schema.
...
export async function createUser(u){
 ...
 const { rows } = await sql`
  INSERT INTO users(full_name, email, email_verified_at, phone, phone_verified_at, whatsapp_number, whatsapp_same_as_mobile, google_id, avatar_url)
  VALUES (${u.fullName}, ${u.email}, ${u.emailVerifiedAt || null}, ${u.phone}, ${u.phoneVerifiedAt || null}, ${u.whatsappNumber || null}, ${u.whatsappSameAsMobile !== false}, ${u.googleId || null}, ${u.avatarUrl || null})
  RETURNING *
 `;
 return rows[0];
}
...
export async function insertBooking(b){
 await ensureSchema();
 const { rows } = await sql`
  INSERT INTO bookings(reference, user_id, name, email, phone, origin, destination, pickup_at, vehicle, passengers, notes, quoted_fare, status, driver_id)
  VALUES (${b.reference}, ${b.userId || null}, ${b.name}, ${b.email}, ${b.phone}, ${b.origin}, ${b.destination}, ${b.pickupAt}, ${b.vehicle}, ${b.passengers}, ${b.notes}, ${b.quotedFare}, ${b.status}, ${b.driverId || null})
  RETURNING *
 `;
 return rows[0];
}
